...
class print_queries:
    ...
    class Snitch(deque):
        ...
        def append(self, item):
            print(item['sql'], item['time'])
            return super().append(item)

    def mocks(self, using):
        ...
        mock_deque = mock.patch.object(connection_, 'queries_log', self.Snitch(maxlen=connection_.queries_log.maxlen))
        return mock_maker, mock_deque
    ...
    def __enter__(self, using='default'):
        self.mock_maker, self.mock_deque = self.mocks(using)
        self.mock_maker.start()
        self.mock_deque.start()

    def __exit__(self, *a, **k):
        self.mock_maker.stop()
        self.mock_deque.stop()

    def __init__(self, func=None, using='default'):
        self.func = func
        self.using = using

    def __call__(self, *args, **kwarg):
        self.mock_maker, self.mock_deque = self.mocks(self.using)
        with self.mock_maker, self.mock_deque:
